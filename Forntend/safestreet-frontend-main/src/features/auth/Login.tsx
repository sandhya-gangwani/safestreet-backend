...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Store user info and JWT token in Zustand store
      loginAction(
        {
          id: user.id,
          email: user.email,
          role: user.role.toUpperCase(), // Ensure capitalized match for RoleGuard (ADMIN, SUPERVISOR, etc.)
          departmentId: user.departmentId,
        },
        accessToken
      );

      // Redirect to main dashboard (which routes to the appropriate child screen via DashboardIndex)
      navigate('/dashboard');
    } catch (err: any) {
      console.error(err);
      setError(
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50 px-4 py-12 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8 bg-white p-8 rounded-2xl shadow-lg border border-slate-100 transition-all duration-300 hover:shadow-xl">
        ...
        {error && (
          <div className="bg-red-50 border-l-4 border-red-500 p-4 rounded-md text-sm text-red-700 animate-shake">
            {error}
          </div>
        )}
